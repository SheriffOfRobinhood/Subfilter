...
for filter_id_1, twod_filter_1 in filter_list.items():
    for filtered_file in filtered_data_files[filter_id_1]:
        
        logger.info(f"Filtering filtered file {filtered_file}")
        input_data = {'file': filtered_file,
                      'level': 'filtered'}
        input_data['ds'] = xr.open_dataset(filtered_file,
                                           chunks={'time':1, 'z_'+opgrid:'auto'})
        ...
        for filter_id_2, twod_filter_2 in filter_list.items():
            ...
            filtered_data_files_2[filter_id_1][filter_id_2].append(
                           filtered_data_file)
            filtered_data_spectra_files_2[filter_id_1][filter_id_2].append(
                           spectra_filt_file)
...
for filter_id_1, d in filtered_data_spectra_files_2.items():
    for filter_id_2, spectra_files in d.items():
        dso = xr.open_mfdataset(spectra_files)
#        print(dso)
        
        kE_km = get_k_Ek(dso, 'spec_2d_f(f(w_on_w)_r)_r')
        
    
        kE_kmz = kE_km.sel(z_w=plot_height, method='nearest')
        sigma_1 = filter_list[filter_id_1].attributes["sigma"]
        sigma_2 = filter_list[filter_id_2].attributes["sigma"]
        kE_kmz.plot(xscale='log', yscale='log', 
                    label=rf'$\sigma_1$={sigma_1} m, $\sigma_2$={sigma_2} m', ax=axa)
        
        if sigma_1 == 50.0 and sigma_2 == 100.0:
            kE_kmz = kE_kmz * 1.2
            kE_kmz.plot(xscale='log', yscale='log', 
                        label=rf'$\sigma_1$={sigma_1} m, $\sigma_2$={sigma_2} m shifted', ax=axa)
...
plt.savefig(odir + 'repeat_filter.pdf')
