...
if test_case == 0:
    ...
    indir = dirroot + 'ug_project_data/Data/test_filter_spectra_RFFT/'
    outdir = dirroot + 'ug_project_data/Data/test_filter_spectra_RFFT/'
    fileroot = 'diagnostics_3d_ts_21600'
    ref_file = 'diagnostics_ts_21600'
    fname = ''
    outtag = '_w_spec'
    plot_height = 250
    max_e_range = (0, 2)
    ...
    outdir = dirroot + 'traj_data/CBL/test_filter_spectra_RFFT/'
...
axa[0].set_xlabel(rf'Wavenumber m$^{-1}$')
...
for sigma in sigmas:
    for alpha in alphas:
        ...
        filt.plot(xscale='log', yscale='log', ax=axa[1],
                  label=rf'$\alpha$={alpha:0.1f}, $\sigma$= {sigma:0.1f} m ')
...
axa[1].set_xlabel(rf'Wavenumber m$^{-1}$')
axa[1].set_ylabel(r'$G(k)\times G(k)^*$')

#%
idl_filt.plot(xscale='log', yscale='linear', ax=axa[2], label='Ref 5 m')
